...
void apply_stencil(const int radius, const double stddev, const int rows, const int cols, pixel * const in, pixel * const out) {
	...
	int i, j;
	//#pragma omp parallel for shared(out, kernel, in) private(i, j)
	cilk_for( i = 0; i < rows; ++i) {
		int x, y, kx, ky;
		//#pragma omp parallel for shared(out, kernel, in) private(x, y, kx, ky)
		for(j = 0; j < cols; ++j) {
			const int out_offset = i + (j*rows);
			// For each pixel, do the stencil
			for( x = i - radius, kx = 0; x <= i + radius; ++x, ++kx) {
				for( y = j - radius, ky = 0; y <= j + radius; ++y, ++ky) {
...
void apply_prewitt(const int rows, const int cols, pixel * const in, pixel * const out) {
	//const int dim = radius*2+1;
	//double kernel[dim*dim];
	//gaussian_kernel(dim, dim, stddev, kernel);
	const int radius = 1;
	...
	double preKy[3*3];
	double preKx[3*3];
	prewittX_kernel(3, 3, preKx);
	prewittY_kernel(3, 3, preKy);
			...
			for( x = i - radius, kx = 0; x <= i + radius; ++x, ++kx) {
				for( y = j - radius, ky = 0; y <= j + radius; ++y, ++ky) {
					if(x >= 0 && x < rows && y >= 0 && y < cols) {
						...
						const int k_offset = kx + (ky*dim);
						//double intensity = (in[in_offset].red + in[in_offset].green + in[in_offset].blue)/3.0;
						intensex += preKx[k_offset] * (in[in_offset].red + in[in_offset].green + in[in_offset].blue)/3;
						intensey += preKy[k_offset] * (in[in_offset].red + in[in_offset].green + in[in_offset].blue)/3;
					}
				}
			}
			double res = sqrt(intensex * intensex * intensey * intensey);
			out[out_offset].red = res;
			out[out_offset].green = res;
			out[out_offset].blue = res;
		}
	}	
...
int main( int argc, char* argv[] ) {
	...
	apply_prewitt(rows, cols, outPixels, outPixels2);
